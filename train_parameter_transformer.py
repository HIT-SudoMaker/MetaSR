...
from models.parameter_transformer import ParameterTransformer

# 设置train_parameter_transformer过程
def train_parameter_transformer(train_dataset: Dataset, val_dataset: Dataset, batch_size: int, n_epochs: int, learning_rate: float, model: nn.Module):
    """
    训练ParameterTransformer模型
    ...
    for epoch in range(n_epochs):
        ...
        for batch in tqdm(train_loader):
            ...
            # 计算训练损失
            loss = criterion(parameter_predicted, parameter)
...
if __name__ == "__main__":
    ...
    # 创建保存目录
    os.makedirs("results/parameter_transformer/models", exist_ok=True)
    os.makedirs("results/parameter_transformer/losses", exist_ok=True)
    
    # 设置结果保存路径
    model_save_path = os.path.join("results/parameter_transformer/models", f"{model_name}.pth")
    loss_save_path = os.path.join("results/parameter_transformer/losses", f"{model_name}.csv")
    ...
    # 创建模型
    model = ParameterTransformer(n_heads=16, n_layers=8, dropout=0.1)
    
    # 训练模型
    best_model, train_losses, val_losses = train_parameter_transformer(train_dataset, val_dataset, batch_size, n_epochs, learning_rate, model)
