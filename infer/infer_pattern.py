...
# 设置infer_pattern过程
def infer_pattern(transmittance_target: torch.Tensor, n_samples: int, n_timesteps: int, model: nn.Module, device: torch.device, crop_indicator: bool=True, drc: bool=True):
    """
    推理超表面结构图案
    ...
    # 推理超表面结构图案
    pattern_inferred = diffusion.infer(initial_noise, transmittance_target, model)
