...
class Diffusion:
    ...
    @torch.no_grad()
    def denoise(self, pattern_addnoised: torch.Tensor, transmittance: torch.Tensor, t: torch.Tensor, model: nn.Module):
        """
    ...
    @torch.no_grad()
    def infer(self, pattern_initial: torch.Tensor, transmittance: torch.Tensor, model: nn.Module, threshold: float=0.51):
        """
        执行完整反向去噪过程：x_T -> x_0
        ...
        for i in reversed(range(self.n_timesteps)):
            ...
            pattern_denoised = self.denoise(pattern_denoised, transmittance, t, model)
...
if __name__ == "__main__":
    ...
    class SimpleModel(nn.Module):
        ...
        def forward(self, pattern_addnoised, transmittance, t):
    ...
    # 测试denoise方法
    pattern_denoised = diffusion.denoise(pattern_addnoised, transmittance, t, model)
    print("denoise方法输出的数据形状：")
    print("pattern_denoised:", pattern_denoised.shape)
    print()
    
    # 测试infer方法
    pattern_inferred = diffusion.infer(pattern_addnoised, transmittance, model)
