...
from models.parameter_pnn import ParameterPNN

# 设置train_parameter_pnn过程
def train_parameter_pnn(train_dataset: Dataset, val_dataset: Dataset, batch_size: int, n_epochs: int, learning_rate: float, model: nn.Module):
    """
    训练ParameterPNN模型
...
if __name__ == "__main__":
    ...
    # 创建保存目录
    os.makedirs("results/parameter_pnn/models", exist_ok=True)
    os.makedirs("results/parameter_pnn/losses", exist_ok=True)
    
    # 设置结果保存路径
    model_save_path = os.path.join("results/parameter_pnn/models", f"{model_name}.pth")
    loss_save_path = os.path.join("results/parameter_pnn/losses", f"{model_name}.csv")
    ...
    # 创建模型
    model = ParameterPNN(transmittance_dim=301, pattern_size=32, output_dim=3)
    
    # 训练模型
    best_model, train_losses, val_losses = train_parameter_pnn(train_dataset, val_dataset, batch_size, n_epochs, learning_rate, model)
