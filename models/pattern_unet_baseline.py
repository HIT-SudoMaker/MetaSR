...
class PatternUNet(nn.Module):
    ...
    def forward(self, pattern_addnoised: torch.Tensor, parameter: torch.Tensor, transmittance: torch.Tensor, t: torch.Tensor):
        """
        执行前向传播过程
        ...
        condition = torch.cat([parameter, transmittance, t_processed], dim=1)
...
if __name__ == "__main__":
    ...
    # 创建PatternUNet实例
    model = PatternUNet(input_channels=1, output_channels=1, d_condition=606, use_attention=True).to(device)
